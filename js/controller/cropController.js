    ...
    if (token) {
        $.ajax({
            ...
            success: function (res) {
                console.log("Crop Code:", res);
                // Verify the API response
                if (res) {
                    document.getElementById("cropCode").value = res;
                    document.getElementById("cropCode").readOnly = true;
                } else {
                    console.error("Empty response from API");
                }
            },
            error: function (err) {
                console.error('Failed to fetch next crop code:', err);
                const errorDiv = document.querySelector('.errorMassageId');
                errorDiv.style.display = "block";
                errorDiv.textContent = "Failed to fetch crop code. Please try again.";
            }
        });
    } else {
...
// Get current date and time
        ...
        doc.text("Green Shadow Crop Details Report", pageWidth / 2, 20, {align: "center"});
        doc.setFontSize(10);
